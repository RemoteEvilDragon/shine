...
function fetchListFromFile(fileName)
    ...
    local targetsList = string.split(t,"\n")
...
function checkCompany(element)
    local index = string.find(element,"@")
    if index  then
        local company = string.sub(element,index+1,string.len(element))
...
function getAbstractFilter(filterList)
    local coms = {}
    for _,v in ipairs(filterList) do
        local company = checkCompany(v)
        if company then
            table.insert(coms,company)
        end
    end
    return coms
end

function goFilter(filterList,targetList)
    local filterList = getAbstractFilter(filterList)

    local isMatch ,filteredList = nil,{}
    for _,v in ipairs(targetList) do 
        isMatch = false
        for _,f in ipairs(filterList) do
            if f == checkCompany(v) then
                isMatch = true
                break
            -- else
                -- print(f,v)
...
function getPreElement(preNum,list)
    local newList = {}
...
function printf(fmt, ...)
    PTPrint(string.format(tostring(fmt), ...))
end
...
PTPrint(iskindof(Duck.new(), "Animal")) -- 输出 true
...
function table.cloneBy(t,beginIndex,endIndex)
    local list={}
...
PTPrint(table.keyof(hashtable, "chukong")) -- 输出 comp
...
PTPrint(string.htmlspecialchars("<ABC>"))
-- 输出 &lt;ABC&gt;
...
PTPrint(string.restorehtmlspecialchars("&lt;ABC&gt;"))
-- 输出 <ABC>
...
PTPrint(string.text2html("<Hello>\nWorld"))
-- 输出
-- &lt;Hello&gt;<br />World
...
local input = "hello world"
PTPrint(string.urlencode(input))
-- 输出
-- hello%20world
...
PTPrint(string.urldecode(input))
-- 输出
-- hello world
...
function string.endsWith(rawStr,suffix)
    local len = string.len(suffix)
    local allLen = string.len(rawStr)

    return (string.sub(rawStr,allLen-len+1,allLen) == suffix)
end
